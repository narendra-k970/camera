...
const EmployeeForm: React.FC<EmployeeFormProps> = ({ darkMode }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post('YOUR_PYTHON_BACKEND_URL/register/employee', formData);
  ...
  return (
    <div className={`max-w-md mx-auto p-6 ${darkMode ? 'bg-gray-800 text-gray-100' : 'bg-gray-100'} rounded-xl shadow-lg transition-colors duration-200`}>
      <h2 className={`text-2xl font-bold mb-6 ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>Employee Registration</h2>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className={`block text-sm font-medium ${darkMode ? 'text-gray-200' : 'text-gray-700'}`}>Employee Name</label>
          <input
            type="text"
            value={formData.name}
        ...
        <div>
          <label className={`block text-sm font-medium ${darkMode ? 'text-gray-200' : 'text-gray-700'}`}>Department</label>
          <input
            type="text"
