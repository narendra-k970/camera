  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data) {
        ...
        toast.success(`${activeTab} registered successfully!`, { position: 'top-right' });
      ...
      if (axios.isAxiosError(error)) {
        if (error.code === 'ECONNABORTED') {
          ...
          toast.error('Connection timed out. Please try again.', { position: 'top-right' });
        } else if (!error.response) {
          // alert('Network error. Please check your connection and try again.');
          toast.error('Network error. Please check your connection and try again.', { position: 'top-right' });
        } else {
          // alert(`Registration failed: ${error.response.data?.message || 'Unknown error'}`);
          toast.error(`Registration failed: ${error.response.data?.message || 'Unknown error'}`, { position: 'top-right' });
        }
      } else {
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            type="text"
            value={formData.val2}
            onChange={(e) => setFormData(prev => ({ ...prev, val2: e.target.value }))}
            className={`mt-1 block w-full rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 ${
              darkMode ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-300 text-gray-900'
            }`}
            required
